...
HD95 is computed via a symmetric nearest-neighbour search using `scipy.spatial.cKDTree`, so no external `medpy` dependency is required.

---

## Results

Trained for 200 epochs on a Kaggle T4 GPU (~3.5 hours).  Dice jumped from **0.69 to 0.90** in the first two epochs due to foreground-biased patch sampling, then converged steadily.
